...
Monde::Monde() :
    ...
    textureBuffer(QGLBuffer::IndexBuffer)
{
    this->bornes = Bornes();
...
QString Monde::getSizes() {
    ...
    //txt += "texture : " + QString::number(sizeof(GLfloat) * textureArray.size());
    return txt;
}
    ...
    if (pointArray[a].z() > pointArray[b].z()) {
        if (pointArray[a].z() > pointArray[c].z()) {
            if (pointArray[c].z() > pointArray[b].z()) {
                ajouterTriangle(i, b, c, a, a, b, c);
            } else {ajouterTriangle(i, c, b, a, a, b, c);}
        } else {
            ajouterTriangle(i, b, a, c, a, b, c);
        }
    } else {
        if (pointArray[a].z() > pointArray[c].z()) {
            ajouterTriangle(i, c, a, b, a, b, c);
        } else {
            if (pointArray[c].z() > pointArray[b].z()) {
                ajouterTriangle(i, a, b, c, a, b, c);
            } else {ajouterTriangle(i, a, c, b, a, b, c);}
        }
    }
}

void Monde::ajouterTriangle(int i, int x, int y, int z, int a, int b, int c) {
    indiceArray[i] = x;
    indiceArray[i + 1] = y;
    indiceArray[i + 2] = z;
    int I = 2 * i;
    int j = 0; //if (pointArray[x].z() > 0.8 * bornes.max.z) {j = 1;}
    textureArray[I] =     pointArray[x].x();
    textureArray[I + 1] = pointArray[x].y();
    textureArray[I + 2] = pointArray[y].x();
    textureArray[I + 3] = pointArray[y].y();
    textureArray[I + 4] = pointArray[z].x();
    textureArray[I + 5] = pointArray[z].y();
}
...
void Monde::genererBuffer() {
    ...
    error = glGetError();
    if (!vertexBuffer.isCreated() || error != 0) {std::cout << error << std::endl; throw 1;}
    ...
    error = glGetError();
    if (!indiceBuffer.isCreated() || error != 0) {std::cout << error << std::endl; throw 2;}
    ...
    colorBuffer.release();
    error = glGetError();
    if (!colorBuffer.isCreated() || error != 0) {std::cout << error << std::endl; throw 3;}

    // Color buffer init
    textureBuffer.create();
    textureBuffer.bind();
    textureBuffer.allocate(textureArray.constData(), textureArray.size() * sizeof(GLfloat));
    textureBuffer.release();
    error = glGetError();
    if (!textureBuffer.isCreated() || error != 0) {std::cout << error << std::endl; throw 4;}
}
...
void Monde::libererBuffer() {
    ...
    if (textureBuffer.isCreated()) {textureBuffer.destroy();}
}
...
void Monde::setSizeColorArray() {
    ...
    textureArray = QVector<GLfloat>(12 * (nbX-1) * (nbY-1));
}
