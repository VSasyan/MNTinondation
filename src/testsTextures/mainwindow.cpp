...
void MainWindow::chargerFichier(QString src) {
    ...
    this->messageStatutBarre = new QLabel("Ouverture du MNT...");
    ui->statusBar->addWidget(this->messageStatutBarre);
    ...
    viewer->monde = Monde();
    ThreadOuvrirMNT *thread = new ThreadOuvrirMNT(&viewer->monde, this, src);
...
void MainWindow::chargerFichierFini() {
    viewer->resetView();
    ui->statusBar->removeWidget(this->messageStatutBarre);
    delete this->messageStatutBarre;
...
void MainWindow::setMessageStatutBarre(QString txt) {
    this->messageStatutBarre->setText(txt);
}

void MainWindow::afficherTitre() {
    QString txt;
    if (tempsChargement != -1) {
        txt = "MondeViewer (" + QString::number(tempsChargement) + " s) : " +
